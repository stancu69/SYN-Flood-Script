...
altbanner = """
		      ...
		      LX scrolls their hearts
		     And blows the blood on the wall
		      		       	L X  
"""
...
def main():
	...
	while True:
		sys.stdout.write("\x1b]2;L X \x07")
		sin = input("\033[1;00m[\033[91mLX\033[1;00m]-\033[91m家\033[00m ").lower()
		sinput = sin.split(" ")[0]
		if sinput == "clear":
			os.system ("clear")
			print (altbanner)
			...
			try:
				...
				for link in domains:
					try:
						...
						sfound += 1
						fsubs += 1
						sys.stdout.write("\x1b]2;L X  |{}| F O U N D\x07".format (sfound))
					except socket.error:
						pass
			...
			try:
				...
				while True:
					try:
						...
						sys.stdout.write("\x1b]2;L X |{}ms| D E M O N S\x07".format (stop))
						print ("Sinfull: {}:{} | Time: {}ms [\033[91mUP\033[00m]".format(ip, port, stop))
						sock.close()
						time.sleep(1)
					except socket.error:
						sys.stdout.write("\x1b]2; L X  |TIME OUT| D E M O N S\x07")
